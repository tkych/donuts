### Last Updated : 2012/05/20 23:37:37 tkych
...
それにもかかわらず、「なめらかさ」をゴールに設定し、Donutsを造った一番の理由は _グラフを描くLispプログラマの生産性の向上のため_ です。
以下で詳しく説明します。
...
したがって、 =(&& nil)= も空グラフを生成します。
一見、空グラフの存在は無駄に見えます。
ところが、グラフを再帰的に生成する時に、その真価が明らかになるでしょう(cf. [[2分木]])。
...
*!!バグ!!: with-node, with-edge;let内で生成束縛されるノード、エッジ、グラフが、let内の属性コンテキストではなく、束縛変数の初期出現時の属性コンテキストに影響される場合がある(see. [[ToDo]])。*
...
- *!!バグ修正!!: with-node, with-edge;*
  *let内で生成束縛されるノード、エッジ、グラフが、let内の属性コンテキストではなく、束縛変数の初期出現時の属性コンテキストに影響される場合がある。*
#+BEGIN_SRC lisp
  (with-node (:color :blue :shape :box)
    (let ((a (<> "a")))
      (with-node (:color :red)
        a)))                         ;ノードaの色が赤になってしまう!!
#+END_SRC
