### Last Updated : 2012/06/09 13:32:26 tkych
...
　 *DonutsはCommon LispのためのグラフDSL(または、Graphvizインターフェイス)です。*
Donutsは、S式で記述されたグラフ構造を[[http://www.graphviz.org/dot-language.html][Dot言語]]のコードに変換し、画像としてビューアに出力します。
すべてのLisp機能(REPL、マクロ、関数、CLOSなど)を利用できるので、複雑なグラフでも簡単に描くことができます(cf.[[小さなプログラム --- カレンダー、2分木、コンスセル、有限オートマトン][小さなプログラム]])。
...
したがって、あなたの望みが健全な常識の範囲内であれば、自由にDonutsを使うことができるはずです。
...
　グラフを作成するだけなら、Graphvizなど数あるソフトウェアをそのまま使えば達成できることです。
それにもかかわらず、Donutsをあえて開発した一番の動機は /グラフを描くLispプログラマの生産性の向上のため/ です。

　そこで、Donutsで達成したいゴールは、Common Lispでグラフ作成を *なめらかに* 行えるようにすることです。
「なめらかに」とは *Lispの思考習慣の枠組を切り替えずに* ということを意味します。
つまり、
 /Lispコード内であたかもC言語でコーディングするかのように思考を切り替えず、Lispの思考習慣のまま、グラフ作成を可能にすること/ です。
以下では、なぜこれが生産性に影響するのかについて説明します。

　およそ、どのようなプログラミング言語でコードを書くとしても、プログラマはその言語特有の思考習慣の中で問題を考えます。
言語特有の思考習慣とは、その言語のシンタックスやセマンティックス、設計思想、コミュニティ文化などに根ざしたもので、そのすべてを文章化することは多分できないでしょう。
それゆえ、LispとJavaによる具体例によって、それを見ていきましょう。
...
例えば、Lispでコードを書きながらC言語を使うとしたら、LispよりCの方が実効速度が早いという利点があるからです。
...
　前節で設定した目標を達成するために、どのような指針の基でソフトウェアを構築すると、うまくいくでしょうか？

　採用した構築指針は *Lispの拡張を目指す* というものです。
これは、「いっちょCLOSやloopやformatに匹敵するものをこさえるべぇ」というようなビッグドリームを語っているのでは /ありません/ 。

　ポールグレアム氏が著書On Lispで提唱している *ボトムアップデザイン* によるプログラミング技法を指針として、ソフトウェアを造っていくということです。
...
しかしながら、コンピュータにグラフ作成を命令するプログラムを書くためには、 /そもそもグラフ構造を構築するにはどうするか/ を探さねばなりません。
...
+ <tag-cons>: cf. http://www.graphviz.org/node-shapes.html
    + <tag-cons>ごとに属性の種類が限定されている。
    + <tag-cons>ごとに引数として取ることのできる<tag>の種類が限定されている(ex. tr,td,hr,vrはtableタグ内でのみ用いることができる)。

+ 正確には、すべてのCommon Lisp文法を上述の文法に加えたものがDonuts文法となる。
...
with-系のユーティリティの返り値、[&]によって生成されるクラスタもgraph型のオブジェクトとみなされる。
